...
The aesthetic direction is **muted classical Greek**. Marble, terracotta, warm charcoal. Restrained, confident, warm. Not maximalist, not tech-bro, not trendy. Critically, not in a style that will date in a year. The portfolio is privacy-conscious: no photo, name as wordmark only.
...
- **List rows (projects):** background wash to `--bg-raised` (marble-50). The row's trailing `→` glyph shifts 3px to the right, its color switches from `--fg-subtle` to aegean, and an ember-tipped overlay fades in behind it. The signature move: one warm pixel at the leading edge of a cool line. No scale, no shadow.
...
- **No emoji.** Emoji break the palette and make the site look like every other portfolio.
